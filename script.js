...
async function loadSettingsPage() {
    ...
    try {
        ...
        document.getElementById('enable-2fa').checked = settings.ENABLE_2FA === 'true';
        document.getElementById('enable-logging').checked = settings.ENABLE_LOGGING === 'true';
...
// Corrected saveSettings function to match your HTML IDs and database keys
async function saveSettings() {
    const backofficeApiUrl = document.getElementById('backoffice-api-url').value;
    const syncApiUrl = document.getElementById('sync-api-url').value;
    const devicePassword = document.getElementById('device-password').value;
    
    // Get the new API credentials
    ...
    if (!backofficeApiUrl || !syncApiUrl || !devicePassword || !apiEmail || !apiPassword) {
        showNotification('Please fill all required API fields!', 'error');
        return;
    }
    ...
    try {
        ...
        await window.electronAPI.setSetting('SYNC_API_URL', syncApiUrl);
        ...
        await window.electronAPI.setSetting('STORE_NAME', storeName);
        await window.electronAPI.setSetting('TIMEZONE', timezone);
        await window.electronAPI.setSetting('ENABLE_2FA', enable2fa.toString());
        await window.electronAPI.setSetting('ENABLE_LOGGING', enableLogging.toString());
...
// This is no longer needed since settings are loaded via the IPC handler
